...
contract MultiSigWallet {
    ...
    function _distributeFees(bytes[] calldata signatures) private {
        ...
        allocation[firstApprover] += feeFirstApprover;

        // Distribute fee to the executor
        allocation[msg.sender] += feeExecutor;

        // Distribute equal fee to the rest of the approvers
        for (uint256 i = 1; i < signatures.length; i++) {
            address approver = recover(getTransactionHash(nonce, msg.sender, 0, ""), signatures[i]);
